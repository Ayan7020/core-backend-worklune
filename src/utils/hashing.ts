...
export const getHash = (inputString: string): getHashReturnInterface => {
  ...
  const hash = crypto.pbkdf2Sync(inputString, salt, ITERATIONS, KEY_LENGTH, DIGEST).toString("hex");

  return { salt, hash };
};

export const hashWithoutSalt = (token: string): string => {
  return crypto.createHmac(DIGEST, REFRESH_TOKEN_SECRET).update(token).digest("hex");
};
...
export const verifyHash = (inputString: string, storedHash: string, salt: string): Boolean => {
  ...
  const inputHash = crypto
    .pbkdf2Sync(inputString, salt, ITERATIONS, KEY_LENGTH, DIGEST)
    .toString("hex");
